...
ylabels = [
    r"$\langle \tilde{O}_1(t)\rangle$",
    r"$\langle \tilde{O}_2(t)\rangle$",
    r"$\langle \tilde{O}_3(t)\rangle$",
]
...
for ax, sig, ylabel, panel_label in zip(
    axs,
    signals,
    ylabels,
    panel_labels,
    ...
    # Set ylims to min and max
    ax.set_ylim(np.min(sig), np.max(sig))

    ax.set_xlim(0)


axs[-1].set_xlabel(r"$\Gamma t$")
...
# Save
save_name = f"time_series_N_{N}_Gamma_{Gamma:.2e}"
